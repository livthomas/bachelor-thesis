...
The core jBPM engine provides a simple KIE\footnotemark\footnotetext{KIE stands for Knowledge Is Everything.} API \cite{jbpm6api} to control the execution of business processes.
To start working with processes it is first necessary to set up a runtime environment.
The most important thing a user has to specify is a list of process definition files.
After that a runtime manager, which is resposible for delivering instances of the runtime engine, can be created.
...
\subsection{Human Task Service}
The Human Task Service (or simply Task Service) is a special part of the engine responsible for human task management.
...
%The Task Service provides a simple API 
...
jBPM Console is a modular application.
...
There are many various options how a mobile version of the jBPM Console can be implemented.
