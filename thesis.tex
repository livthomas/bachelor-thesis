...
That means it can be both embedded in an application or run as a service. %which users can connect to through the web-based UI or remote APIs.
...
After the editing is done a project can be build and deployed so the individual processes within it become ready to run.
...
If a process is started a user is prompted to fill in the process form which consists of some input fields. %if there are any defined.
